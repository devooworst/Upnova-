      ...
      <div className="flex items-start gap-6">
        ...
        <div className="min-w-0 flex-1">
          ...
          {category === "Services" && (
            <div className="grid gap-4 md:grid-cols-2">
              {creators
                .filter((c) => c.startingAt)
                .flatMap((c) =>
                  services.slice(0, 2).map((s, i) => ({ c, s, key: `${c.id}-${s.id}-${i}` }))
                )
                .slice(0, 6)
                .map(({ c, s, key }) => (
                  <article key={key} className="card p-5 transition hover:border-zinc-600">
                    ...
                    <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                      <p className="text-sm text-zinc-400">
                        Starting at{" "}
                        <span className="text-base font-bold text-lime-400">${s.startingAt}</span>
                      </p>
                      <button className="btn-lime px-4 py-1.5 text-xs">Hire Me</button>
                    </div>
                  </article>
                ))}
            </div>
