...
async function wipe() {
  ...
  const seedUsers = await db.select().from(t.users).where(eq(t.users.isSeed, true)).all();
  ...
  if (ids.length) {
    ...
    const seedProjects = (await db.select().from(t.projects).all())
      .filter((p) => seedSet.has(p.clientId) || seedSet.has(p.creatorId))
      .map((p) => p.id);
    if (seedProjects.length) {
      await db.delete(t.reviews).where(inArray(t.reviews.projectId, seedProjects)).run();
      await db.delete(t.payments).where(inArray(t.payments.projectId, seedProjects)).run();
      await db.delete(t.extensionRequests).where(inArray(t.extensionRequests.projectId, seedProjects)).run();
      await db.delete(t.projectMilestones).where(inArray(t.projectMilestones.projectId, seedProjects)).run();
      await db.delete(t.projects).where(inArray(t.projects.id, seedProjects)).run();
    }
    await db.delete(t.payments).where(inArray(t.payments.payerId, ids)).run();
    await db.delete(t.payments).where(inArray(t.payments.payeeId, ids)).run();
    await db.delete(t.reviews).where(inArray(t.reviews.authorId, ids)).run();
    await db.delete(t.reviews).where(inArray(t.reviews.subjectId, ids)).run();
    await db.delete(t.bookings).where(inArray(t.bookings.clientId, ids)).run();
    await db.delete(t.bookings).where(inArray(t.bookings.providerId, ids)).run();
    await db.delete(t.reports).where(inArray(t.reports.reporterId, ids)).run();
    await db.delete(t.notifications).where(inArray(t.notifications.userId, ids)).run();

    // conversations aren't owned — delete ones whose members are all seeds
    const convs = await db.select().from(t.conversationMembers).all();
    ...
    const convIds = Array.from(byConv.entries())
      ...
      .map(([cid]) => cid);
    if (convIds.length) await db.delete(t.conversations).where(inArray(t.conversations.id, convIds)).run();
    ...
    await db.delete(t.communities).where(eq(t.communities.isSeed, true)).run();
    await db.delete(t.communities).where(inArray(t.communities.createdById, ids)).run();
    ...
    const liveSessions = (await Promise.all((await db
      .select({ token: t.sessions.token, expiresAt: t.sessions.expiresAt, userId: t.sessions.userId })
      .from(t.sessions)
      .all())
      .map(async (srow) => {
        const u = await db.select({ handle: t.users.handle }).from(t.users).where(eq(t.users.id, srow.userId)).get();
    ...
    await db.delete(t.users).where(inArray(t.users.id, ids)).run();
  } else {
    await db.delete(t.communities).where(eq(t.communities.isSeed, true)).run();
  }
  await db.delete(t.campuses).where(eq(t.campuses.isSeed, true)).run();
  console.log(`Wiped ${ids.length} seed users and their records.`);
}

async function seed() {
  const existing = await db.select().from(t.users).where(eq(t.users.handle, "devin")).get();
  if (existing) {
  ...
  for (const [handle, program] of [["devin", "Cybersecurity"], ["nia", "Communications"], ["imani", "Business"], ["omar", "Mathematics"]] as const) {
    await db.insert(t.campusVerifications)
      .values({
  ...
  for (const [a, b] of followPairs)
    await db.insert(t.follows).values({ followerId: uid[a], followingId: uid[b] }).run();
  ...
  for (const c of communityDefs) {
    ...
    await db.insert(t.communities)
      ...
      .run();
    await db.insert(t.communityMembers).values({ communityId, userId: uid[c.owner], role: "owner", status: "active" }).run();
  }
  ...
  for (const [slug, handle] of memberships)
    await db.insert(t.communityMembers).values({ communityId: cid[slug], userId: uid[handle], status: "active" }).run();
  ...
  await db.insert(t.communityMembers).values({ communityId: cid["bowie-cybersecurity-study-group"], userId: uid["nia"], status: "pending" }).run();

  // per-community aliases + anon codes (random per community — imani is 482
  // in Late Night and a different number anywhere else)
  const setMember = async (slug: string, handle: string, patch: Record<string, unknown>) =>
    await db.update(t.communityMembers)
      .set(patch)
      .where(and(eq(t.communityMembers.communityId, cid[slug]), eq(t.communityMembers.userId, uid[handle])))
      .run();
  await setMember("late-night-conversations", "nia", { alias: "CampusQueen", lastIdentity: "alias" });
  await setMember("late-night-conversations", "imani", { anonCode: "482", lastIdentity: "anonymous" });
  await setMember("late-night-conversations", "omar", { anonCode: "917", lastIdentity: "anonymous" });
  await setMember("late-night-conversations", "devin", { anonCode: "358" });
  await setMember("bowie-anime", "nia", { alias: "CampusQueen" });
  await setMember("bowie-campus-questions", "imani", { anonCode: "274" });
  ...
  for (const p of communityPostDefs) {
    const pId = id();
    cpid[p.key] = pId;
    await db.insert(t.communityPosts)
      .values({
  ...
  for (const cdef of communityCommentDefs) {
    ...
    if (cdef.identity === "alias" && cdef.author === "imani")
      await setMember("bowie-anime", "imani", { alias: "MoonlitPages" });
    await db.insert(t.communityComments)
      .values({
  ...
  for (const [post, who] of [["lonely", "nia"], ["lonely", "devin"], ["anime", "devin"], ["anime", "omar"], ["print", "nia"]] as const)
    await db.insert(t.communityReactions).values({ postId: cpid[post], userId: uid[who] }).run();
  ...
  await db.insert(t.identityReveals)
    .values({
  ...
  await db.insert(t.identityReveals)
    ...
    .run();
  await db.update(t.profiles).set({ revealIdentityMode: "always_profile" }).where(eq(t.profiles.userId, uid["omar"])).run();
  ...
  for (const o of oppDefs) {
    const oppId = id();
    oid[o.title] = oppId;
    await db.insert(t.opportunities)
      .values({
  ...
  const workBeat = id();
  await db.insert(t.works).values({
  ...
  const dispPs5 = id();
  await db.insert(t.disputes).values({
  ...
  const evPs5 = async (kind: string, note: string, h: number, actor: string | null) =>
    await db.insert(t.orderEvents).values({ id: id(), orderId: orderPs5, actorId: actor, kind, note, createdAt: hoursAgo(h) }).run();
  await evPs5("created", "Game Console — 1TB (Disc Edition) ×1 · listing price $500", 24 * 5, uid["rachel"]);
  await evPs5("paid", "$525.00 secured (incl. fee) — held until completion", 24 * 5 - 1, uid["rachel"]);
  await evPs5("shipped", "UPS 1Z999AA10123456784 · serial recorded (private) · 9.8 lb (weight is context, not proof of contents)", 24 * 4, uid["marcusj"]);
  await evPs5("delivered", "Carrier confirmed delivery", 30, null);
  await evPs5("protection_started", "96h buyer-protection window", 30, null);
  await evPs5("disputed", "wrong_package_contents", 22, uid["rachel"]);
  await evPs5("evidence", "Buyer: box of books, photos attached", 20, uid["rachel"]);
  await evPs5("evidence", "Seller: serial + packing evidence, tape mismatch claim", 16, uid["marcusj"]);
  await evPs5("escalated", "Seller contested — sent to platform review", 16, uid["marcusj"]);
  ...
  const bowieId = (await db.select().from(t.campuses).all()).find((c) => c.name.includes("Bowie"))!.id;
  const mkListing = async (v: Record<string, unknown>) => {
    const lid = id();
    await db.insert(t.campusListings).values({ id: lid, campusId: bowieId, isSeed: true, ...v } as never).run();
    return lid;
  };
  await mkListing({ sellerId: uid["omar"], title: "Calc I & II Textbook (Stewart, 9th ed.)", type: "fixed", price: 25, category: "textbooks", condition: "good", description: "Highlighting in ch. 3-5, otherwise clean. Campus pickup at the library.", meetSpot: "Thurgood Marshall Library lobby" });
  await mkListing({ sellerId: uid["imani"], title: "Mini fridge — moving out", type: "free", price: null, category: "dorm & housing", condition: "good", description: "Works perfectly, just can't take it home. First come first served.", meetSpot: "Towers Hall front desk" });
  const auctionId = await mkListing({ sellerId: uid["omar"], title: "Dorm futon, barely used", type: "auction", price: 20, category: "dorm & housing", condition: "like_new", description: "Folds flat, dark gray. Auction ends this week — pickup only.", bidIncrement: 5, auctionEndsAt: daysFromNow(3), meetSpot: "Christa McAuliffe Hall" });
  await db.insert(t.bids).values({ id: id(), listingId: auctionId, bidderId: uid["nia"], amount: 20, createdAt: hoursAgo(10) }).run();
  await db.insert(t.bids).values({ id: id(), listingId: auctionId, bidderId: uid["imani"], amount: 25, createdAt: hoursAgo(4) }).run();
  const calcListing = await mkListing({ sellerId: uid["devin"], title: "TI-84 Plus CE — available to borrow", type: "borrow", price: null, category: "electronics", condition: "like_new", description: "Exam season special: borrow it, pass, bring it back. Charger included.", maxBorrowDays: 7, meetSpot: "Student Center" });
  await mkListing({ sellerId: uid["nia"], title: "MacBook charger (USB-C) for tonight", type: "need_borrow", price: null, category: "electronics", description: "Mine died and my essay is due at midnight. Need it just for tonight — I'll return it first thing tomorrow." });
  ...
  await db.insert(t.loans).values({
    ...
    conversationId: await makeConversation("devin", "omar", [
  ...
  await db.insert(t.loans).values({
    ...
    conversationId: await makeConversation("devin", "imani", [
  ...
  const shootApp = async (handle: string, roleId: string, status: string, message: string) => {
    await db.insert(t.applications).values({
      id: id(), opportunityId: shootId, applicantId: uid[handle], roleId,
      message, availability: "yes", status, createdAt: hoursAgo(30),
    }).run();
  };
  await shootApp("marcusj", "photo", "confirmed", "Campaign work is my bread and butter — recent brand shoots on my profile.");
  await shootApp("omar", "photo", "submitted", "Portfolio has two lookbooks from campus brands. Would love the shot.");
  await shootApp("maya", "model", "submitted", "Runway and print experience — comp card on request.");
  await shootApp("nia", "model", "shortlisted", "Modeled for two local brands last season — flexible on time.");
  await shootApp("tj", "model", "submitted", "New to modeling but very comfortable on camera.");
  await shootApp("imani", "mua", "selected", "Beauty is my whole business — bridal and editorial kits ready.");
  await shootApp("lena", "stylist", "submitted", "I art-direct brand identities — styling the racks would be fun.");
  await db.insert(t.posts).values({
  ...
  for (const p of postDefs) {
    ...
    await db.insert(t.posts)
      ...
      .run();
    for (const liker of p.likes) await db.insert(t.likes).values({ postId, userId: uid[liker] }).run();
    for (const [commenter, text] of p.comments)
      await db.insert(t.comments).values({ id: id(), postId, authorId: uid[commenter], body: text, createdAt: hoursAgo(p.hours - 1) }).run();
  }

  /* --------------------------- conversations --------------------------- */
  async function makeConversation(a: string, b: string, msgs: [string, string, number][]) {
    const convId = id();
    await db.insert(t.conversations).values({ id: convId }).run();
  ...
  const convJordan = await makeConversation("jordanmiles", "devin", [
  ...
  await makeConversation("lena", "devin", [
  ...
  const convNia = await makeConversation("nia", "devin", [
  ...
  await db.insert(t.extensionRequests).values({
  ...
  await db.update(t.projects).set({ state: "extension_requested" }).where(eq(t.projects.id, projAva)).run();
  ...
  const convMarcus = await makeConversation("marcusj", "devin", [
  ...
  /* ------------------------------ bookmarks ------------------------------ */
  await db.insert(t.bookmarks).values([
  ...
  await mkBooking({ service: "devin:Music Production", client: "maya", provider: "devin", title: "Music Production", starts: at(1, 14), dur: 180, price: 300, loc: "Devin's studio", status: "pending" });
  await mkBooking({ service: "devin:Music Production", client: "marcusj", provider: "devin", title: "Music Production", starts: at(5, 10), dur: 240, price: 300, loc: "Devin's studio", status: "confirmed" });
  await mkBooking({ service: "devin:Songwriting", client: "kofi", provider: "devin", title: "Songwriting", starts: at(8, 15), dur: 120, price: 150, loc: "Remote session", status: "accepted" });
  await mkBooking({ service: "devin:Music Production", client: "jordanmiles", provider: "devin", title: "Music Production", starts: at(-4, 11), dur: 240, price: 300, loc: "Devin's studio", status: "completed" });
  await mkBooking({ service: "devin:Songwriting", client: "tj", provider: "devin", title: "Songwriting", starts: at(-9, 13), dur: 120, price: 150, loc: "Remote session", status: "completed" });
  // devin as CLIENT
  await mkBooking({ service: "ava:Event Photography", client: "devin", provider: "ava", title: "Event Photography", starts: at(3, 17), dur: 180, price: 250, loc: "Rooftop — Fells Point", status: "confirmed" });

  // Marcus's confirmed shoot role — the Team-view engagement: a REAL
  // booking (payment pending; Sofia pays through the normal flow)
  const shootConv = await makeConversation("sofia", "marcusj", [
  ...
  await mkBooking({ service: "imani:Gel Nail Set", client: "devin", provider: "imani", title: "Gel Nail Set (gift booking)", starts: at(6, 10), dur: 90, price: 55, loc: "Bowie campus", status: "reschedule_requested", proposed: at(7, 10) });
  await mkBooking({ service: "tj:Event DJ — 4 Hours", client: "devin", provider: "tj", title: "Event DJ — 4 Hours", starts: at(-2, 20), dur: 240, price: 400, loc: "The Assembly Room", status: "cancelled" });
  ...
  await db.insert(t.experiences).values([
  ...
  try {
    ...
    if (fs.existsSync(f)) {
      ...
      for (const p of preserved) {
        ...
        const u = await db.select({ id: t.users.id }).from(t.users).where(eq(t.users.handle, p.handle)).get();
        if (!u) continue;
        await db.insert(t.sessions).values({ id: id(), token: p.token, userId: u.id, expiresAt: new Date(p.expiresAt) }).run();
        restored++;
      }
      fs.unlinkSync(f);
  ...
  for (const q of qaDefs) {
    if (await db.select().from(t.users).where(eq(t.users.handle, q.handle)).get()) continue;
    const qid = id();
    await db.insert(t.users)
      .values({
        id: qid,
...
if (runDirectly) {
  (async () => {
    ...
    process.exit(0); // PGlite keeps the event loop alive — exit explicitly
  })().catch((err) => {
    console.error(err);
    process.exit(1);
  });
