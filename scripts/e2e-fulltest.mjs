...
const BASE = process.env.UPNOVA_URL || "http://localhost:3000";
const login = await fetch(`${BASE}/api/auth/login`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ identifier: "devin", password: "upnova123" }) });
...
console.log(`\nFULL UPNOVA SYSTEM TEST — ${(report.summary.durationMs / 1000).toFixed(1)}s`);
