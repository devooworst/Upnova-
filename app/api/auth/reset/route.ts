...
export async function POST(req: NextRequest) {
  ...
  return guarded(() => {
    ...
    const reset = db.select().from(tables.passwordResets).where(eq(tables.passwordResets.token, token)).get();
    ...
    db.delete(tables.sessions).where(eq(tables.sessions.userId, reset.userId)).run();
