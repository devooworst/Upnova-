...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  return guarded(() => {
    ...
    if (stream.status === "live") assertCanWatch(stream, user.id);
    else if (stream.replayStatus !== "saved" && stream.hostId !== user.id)
      throw new ApiError(404, "This live has ended");

    const canModerate = stream.hostId === user.id || isModerator(stream.id, user.id);
    const msgs = db
      .select()
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  return guarded(() => {
    ...
    if (restrictionOf(stream.id, user.id, "mute"))
    ...
    db.insert(tables.liveMessages).values({ id, streamId: stream.id, userId: user.id, body: text }).run();
    return { ok: true, id };
  });
}
