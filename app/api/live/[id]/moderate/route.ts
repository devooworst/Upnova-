...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  return guarded(async () => {
    ...
    const mod = host || (await isModerator(stream.id, user.id));
    ...
    if (action === "delete_message") {
      ...
      await db.update(tables.liveMessages).set({ deleted: true }).where(eq(tables.liveMessages.id, msg!.id)).run();
    ...
    if (["mute", "unmute", "block", "unblock"].includes(action)) {
      ...
      const existing = await restrictionOf(stream.id, targetId, kind as "mute" | "block");
      if (action === "mute" || action === "block") {
        if (!existing)
          await db.insert(tables.liveRestrictions)
            .values({ id: randomBytes(12).toString("hex"), streamId: stream.id, userId: targetId, kind })
            .run();
      } else if (existing) {
        await db.delete(tables.liveRestrictions).where(eq(tables.liveRestrictions.id, existing!.id)).run();
      }
      return { ok: true };
    }
    ...
    if (action === "add_mod" || action === "remove_mod") {
      ...
      if (action === "add_mod" && !existing)
        (await db.insert(tables.liveModerators).values({ streamId: stream.id, userId: targetId }).run());
      if (action === "remove_mod" && existing)
        await db.delete(tables.liveModerators)
          .where(and(eq(tables.liveModerators.streamId, stream.id), eq(tables.liveModerators.userId, targetId)))
          .run();
      return { ok: true };
    }
