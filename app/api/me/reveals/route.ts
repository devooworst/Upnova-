...
export async function GET() {
  return guarded(() => {
    ...
    const communityNames = new Map(
      db.select({ id: tables.communities.id, name: tables.communities.name, slug: tables.communities.slug }).from(tables.communities).all().map((c) => [c.id, c])
    );

    const iFollow = new Set(
      db.select({ id: tables.follows.followingId }).from(tables.follows).where(eq(tables.follows.followerId, user.id)).all().map((r) => r.id)
    );
    const followMe = new Set(
      db.select({ id: tables.follows.followerId }).from(tables.follows).where(eq(tables.follows.followingId, user.id)).all().map((r) => r.id)
    );
