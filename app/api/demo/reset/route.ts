...
export async function POST(req: NextRequest) {
  ...
  return guarded(async () => {
    ...
    const me = (await db.select().from(tables.users).where(eq(tables.users.id, user.id)).get())!;
    ...
    if (kind === "purchase") {
      ...
      if (ids.length) {
        await db.delete(tables.orderEvents).where(inArray(tables.orderEvents.orderId, ids)).run();
        for (const o of rows) {
          await db.delete(tables.payments).where(eq(tables.payments.orderId, o.id)).run();
          await db.delete(tables.orders).where(eq(tables.orders.id, o.id)).run();
        }
      }
      return { reset: rows.length };
    }
    if (kind === "application") {
      ...
      for (const r of rows) await db.delete(tables.applications).where(eq(tables.applications.id, r.app.id)).run();
      return { reset: rows.length };
    }
