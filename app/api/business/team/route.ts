...
export async function POST(req: NextRequest) {
  ...
  return guarded(async () => {
    ...
    const target = body.personId
      ? await db.select().from(tables.users).where(eq(tables.users.id, String(body.personId))).get()
    ...
    if (!(await interactionBasis(user.id, target.id)))
    ...
    const existingRow = await db
      ...
      .get();
    if (!existingRow || existingRow!.status !== "active") await assertCapacityById(user.id, "teamMembers");
    const wantAdmin = !!body.isAdmin;
    if (wantAdmin && !existingRow?.isAdmin) await assertCapacityById(user.id, "admins");
    ...
    let rowId: string;
    if (existing) {
      await db.update(tables.businessTeam)
      ...
      await db.insert(tables.businessTeam).values({ id: rowId, businessId: user.id, personId: target.id, title, compensation, notes, isAdmin: wantAdmin }).run();
    }
    ...
    const row = (await db.select().from(tables.businessTeam).where(eq(tables.businessTeam.id, rowId)).get())!;
