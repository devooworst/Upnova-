...
async function ownedRow(id: string, businessId: string) {
  const row =await  await db.select().from(tables.businessTeam).where(eq(tables.businessTeam.id, id)).get();
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  return guarded(async () => {
    ...
    if (body.isAdmin != null) {
      ...
      if (flag && !(await row).isAdmin) await assertCapacityById(user.id, "admins");
      patch.isAdmin = flag;
    }
    if (body.status != null) {
    ...
    await db.update(tables.businessTeam).set(patch).where(eq(tables.businessTeam.id, (await row).id)).run();
    const fresh = (await db.select().from(tables.businessTeam).where(eq(tables.businessTeam.id, (await row).id)).get())!;
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  return guarded(async () => {
    const user = await requireUser();
    const row = ownedRow(params.id, user.id);
    if ((await row).status !== "inactive") {
      await db.update(tables.businessTeam).set({ status: "inactive", endedAt: new Date() }).where(eq(tables.businessTeam.id, (await row).id)).run();
      await notify({
        userId: (await row).personId,
        actorId: user.id,
        type: "team_added",
    ...
    return { id: (await row).id, status: "inactive" };
  });
}
