...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  return guarded(async () => {
    ...
    const loan = await db.select().from(tables.loans).where(eq(tables.loans.id, params.id)).get();
    ...
    const set = async (patch: Partial<typeof tables.loans.$inferInsert>) =>
      await db.update(tables.loans).set(patch).where(eq(tables.loans.id, loan.id)).run();
    const cond = () => ({
    ...
    if (action === "approve") {
      ...
      await notify({ userId: other, actorId: user.id, type: "order", title: `Borrow approved — ${loan.itemTitle}`, body: `Arrange the campus pickup in Messages. Return by ${due}.${loan.deposit ? ` Agreed refundable deposit: $${loan.deposit}.` : ""}`, href: "/campus/market?loans=1" });
    ...
    if (action === "decline") {
      ...
      await set({ status: "declined" });
      await notify({ userId: other, actorId: user.id, type: "order", title: `Borrow request declined — ${loan.itemTitle}`, body: "", href: "/campus/market", priority: "low" });
    ...
    if (action === "handoff") {
      ...
      await set({ status: "borrowed", startAt: new Date(), conditionBefore: JSON.stringify(cond()) });
      await notify({ userId: other, actorId: user.id, type: "order", title: `Borrowed — ${loan.itemTitle}`, body: `Return by ${due}. Condition documented at handoff.`, href: "/campus/market?loans=1" });
    ...
    if (action === "request_extension") {
      ...
      const listing = loan.listingId ? await db.select().from(tables.campusListings).where(eq(tables.campusListings.id, loan.listingId)).get() : null;
      ...
      await set({ extensionUntil: until, counterUntil: null });
      await notify({ userId: other, actorId: user.id, type: "order", title: `Extension requested — ${loan.itemTitle}`, body: `Until ${fmt(until)}. Approve, decline, or propose a different date in Borrowing.`, href: "/campus/market?loans=1", priority: "high" });
    ...
    if (action === "extension_decide") {
      ...
      await set(approve ? { dueAt: loan.extensionUntil, extensionUntil: null, counterUntil: null, dueSoonNotified: false, overdueNotified: false } : { extensionUntil: null });
      await notify({ userId: other, actorId: user.id, type: "order", title: approve ? `Extension approved — ${loan.itemTitle}` : `Extension declined — ${loan.itemTitle}`, body: approve ? `New agreed return: ${fmt(loan.extensionUntil)}` : `Still due ${due}. You can message or propose a different date.`, href: "/campus/market?loans=1" });
    ...
    if (action === "extension_counter") {
      ...
      await set({ counterUntil: until, extensionUntil: null });
      await notify({ userId: other, actorId: user.id, type: "order", title: `Different return date proposed — ${loan.itemTitle}`, body: `${fmt(until)} instead. Accept or decline in Borrowing.`, href: "/campus/market?loans=1", priority: "high" });
    ...
    if (action === "counter_decide") {
      ...
      await set(accept ? { dueAt: loan.counterUntil, counterUntil: null, extensionUntil: null, dueSoonNotified: false, overdueNotified: false } : { counterUntil: null });
      await notify({ userId: other, actorId: user.id, type: "order", title: accept ? `New return date agreed — ${loan.itemTitle}` : `Proposed date declined — ${loan.itemTitle}`, body: accept ? `Both of you now see: return by ${fmt(loan.counterUntil)}` : `Still due ${due}.`, href: "/campus/market?loans=1" });
    ...
    if (action === "mark_returned") {
      ...
      await set({ status: "return_claimed", returnedAt: new Date() });
      await notify({ userId: other, actorId: user.id, type: "order", title: `Return claimed — ${loan.itemTitle}`, body: "Confirm the return and the condition to complete the loan.", href: "/campus/market?loans=1" });
    ...
    if (action === "confirm_return") {
      ...
      await set({ status: problem ? "returned_disputed" : "completed", conditionAfter: JSON.stringify(cond()), returnedAt: backAt, returnedLate: late });
      if (problem) {
        await db.insert(tables.reports)
          .values({
            ...
            signals: JSON.stringify((async () => {
              const hist = await db.select().from(tables.loans).where(eq(tables.loans.borrowerId, loan.borrowerId)).all();
          ...
          .run();
        await notify({ userId: other, actorId: user.id, type: "order", title: `Return problem reported — ${loan.itemTitle}`, body: "The before/after condition records go to review. Nothing is decided automatically.", href: "/campus/market?loans=1", priority: "high" });
      } else {
        await notify({ userId: other, actorId: user.id, type: "order", title: `Loan completed — ${loan.itemTitle}`, body: `Returned in good shape. Thanks for the campus assist!${loan.deposit ? ` Refundable deposit ($${loan.deposit}) returns to the borrower.` : ""}`, href: "/campus/market?loans=1" });
      }
